...
void DrawCandidates(cv::Mat im1, cv::Mat im2, 
  ...
  ResizeAndDisplay(name, im12, 0.5, 3);
  //cv::waitKey(0);
...
int CountPositive(const std::vector<std::vector<bool>> &v) {
  int count = 0;
  for (size_t i = 0; i < v.size(); i++) {
    for (size_t j = 0; j < v[i].size(); j++) {
      if (v[i][j] == true) {
...
AngMatcher::AngMatcher(Image* im1_, Image* im2_,
                       FisheyeLens* lens_,
                       cv::Mat F_,
  ...
  candidates_bool_ = std::vector<std::vector<std::vector<bool>>>(num_methods_);
...
void AngMatcher::Match(std::string method,
  ...
  if (method == "epiline") {
    ...
    candidates_[method_map_[method]] = MatchAngle3D(th_geom, bCrossVerification);
  }
  else {
...
std::vector<std::vector<double>> AngMatcher::GetMatches(std::string method, int kp){
  if (kp == -1)
    return candidates_[method_map_[method]];
  else {
    std::vector<std::vector<double>> kp_matches;
    kp_matches.push_back(candidates_[method_map_[method]][kp]);
    return kp_matches;
  }
}


std::vector<std::vector<bool>> AngMatcher::GetMatchesBool(std::string method, int kp){
  if (kp == -1)
    return candidates_bool_[method_map_[method]];
  else {
    std::vector<std::vector<bool>> kp_matches;
    kp_matches.push_back(candidates_bool_[method_map_[method]][kp]);
    return kp_matches;
  }
}
...
int AngMatcher::GetMatchesNN(std::string method, int kp){
  for (auto match : nn_candidates_[method_map_[method]]){
    if (match.queryIdx == kp){
...
int AngMatcher::GetMatchesDesc(std::string method, int kp){
  for (auto match : desc_matches_[method_map_[method]]){
    if (match.queryIdx == kp){
      return match.trainIdx;
    }
  }
...
void AngMatcher::ViewCandidatesCompare(std::string method1, std::string method2, int kp) {
  cv::Point3f pt0(0, -gmlines1[kp][2] / gmlines1[kp][1], 0.);
  cv::Point3f pt1(lx, -(gmlines1[kp][2] + gmlines1[kp][0] * lx) / gmlines1[kp][1], 0.);
  cv::Vec3f line = EquationLine(cv::Point2f(pt0.x, pt0.y), cv::Point2f(pt1.x, pt1.y));
...
/*
std::string AngMatcher::ReportCandidates(std::string method, int kp_id) {
  std::string report = "Report match " + method + " " + std::to_string(kp_id + 1) + " / " + std::to_string(vkps1.size()) + " (all/nn/desc): "
                                       + std::to_string(candidates_[method_map_[method]][kp_id].size()) + " / " 
                                       + std::string(nn_matches_[method_map_[method]][kp_id].size()) + " / " 
                                       + std::string(desc_matches_[method_map_[method]][kp_id].size());
}
*/
...
std::vector<std::vector<double>> AngMatcher::MatchAngle3D(float th, bool bCrossVerification, 
  ...
  for (size_t i = 0; i < vkps1.size(); i++) {
    ...
    if (points.size() > 0 && bDraw) {
      std::string name = "Epiline " + std::to_string(i + 1) + " / " + std::to_string(vkps1.size()) + 
                         " - " + std::to_string(points.size()) + " candidates";
      DrawCandidates(im1, im2, line, kpoints1[i], points, name);
    }
  }
...
std::vector<std::vector<double>> AngMatcher::MatchAngle3D(double th, bool bCrossVerification) {
  
  // Look for match candidates
  std::vector<std::vector<double>> candidates = std::vector<std::vector<double>>(
    vkps1.size(), std::vector<double>(vkps2.size(), -1.0)
  );
  std::vector<std::vector<bool>> candidates_bool = std::vector<std::vector<bool>>(
    vkps1.size(), std::vector<bool>(vkps2.size(), false)
  );
  
  for (unsigned int i=0; i<image1->kps_.size(); i++) {
    cv::Point2f pt1 = image1->kps_[i].pt;
    std::vector<double> pt1_cil_cam = lens->Compute3D(pt1.x, pt1.y, true);
    cv::Point3d pt1_cam = lens->CilToCart(pt1_cil_cam[0], pt1_cil_cam[1]);
    cv::Point3d pt1_w = image1->PointGlobal(pt1_cam);
    cv::Vec4d pi1 = am::EquationPlane(pt1_w, co1g, co2g);

    for (unsigned int j=0; j<image2->kps_.size(); j++) {
      cv::Point2f pt2 = image2->kps_[j].pt;
      std::vector<double> pt2_cil_cam = lens->Compute3D(pt2.x, pt2.y, true);
      cv::Point3d pt2_cam = lens->CilToCart(pt2_cil_cam[0], pt2_cil_cam[1]);
      cv::Point3d pt2_w = image2->PointGlobalRotation(pt2_cam);

      // Check if point is under angle threshold
      double angle1 = am::AngleLinePlane(pi1, pt2_w);
      if (angle1 < th) {

        if (bCrossVerification) {
          cv::Vec4f pi2 = am::EquationPlane(image2->PointGlobalTranslation(pt2_w), co1g, co2g);
          double angle2 = am::AngleLinePlane(pi2, pt1_w);
          if (angle2 < th) {
            candidates[i][j] = angle1 + angle2;
            candidates_bool[i][j] = true;
          }
        } else {
          candidates[i][j] = angle1;
          candidates_bool[i][j] = true;
        }
      }
    }
  ...
  //candidates_[method_map_["angle3d"]] = candidates;
  candidates_bool_[method_map_["angle3d"]] = candidates_bool;
